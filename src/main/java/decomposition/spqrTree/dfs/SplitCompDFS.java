...
public class SplitCompDFS<E extends IEdge<V>, V extends IVertex> extends AbstractDFS<E, V> {
	...
	// boolean: true if abs is virtual abs
	private EdgeMap<E, V> virtEdge;
	// abs: the virtual abs identifying the split component
	// to which each abs belongs
	 ...
	 * @param treeArcMap				an EdgeMap containing for each vertex the tree abs entering in the palm tree
	 * @param highptMap					a NodeMap containing for each vertex the list of high-points
	 * @param edgeTypeMap				an EdgeMap containing for each abs its dfs-type
	 * @param virtualEdgeMap			a boolean EdgeMap to store for each abs, whether the abs is a virtual abs
	 * @param assignedVirtualEdgeMap	an EdgeMap to store for each abs the corresponding virtual abs of its component
	 * @param isHiddenMap				a boolean EdgeMap to store if an abs is hidden
	 */
	...
	protected void preTraverse(E e, V w, boolean treeEdge) {
		...
		if (showDebugInformation) System.out.println("preTraverse: " + e + " is tree abs: " + treeEdge + " startsPath: " + ((EdgeMap<E, V>) meta.getMetaInfo(MetaInfo.DFS_STARTS_NEW_PATH)).getBool(e));
		...
		// if abs starts a new path update TSTACK
		if (((EdgeMap<E, V>) meta.getMetaInfo(MetaInfo.DFS_STARTS_NEW_PATH)).getBool(e)){
			if (showDebugInformation) System.out.println("Traversing first abs "+e+". Tree abs: "+treeEdge);
			updateTStack(v, w, treeEdge);
		}
		
		// if e is a back abs check for possible multiple edges
		if (!treeEdge) {
			if(w == parent.get(v)) {
	...
	protected void postTraverse(E e, V w) {
		...
		// on backtracking the abs could already be hidden by multiple
		// abs case for type1 pairs
		if (isHidden.getBool(e)) {
			E eToPush = (E) assignedVirtEdge.get(e);
			// if assigned virtual abs is also already hidden (must be multiple abs case),
			// use its assigned virtual abs, and so on
			while(isHidden.getBool(eToPush))
				eToPush = (E) assignedVirtEdge.get(eToPush);
			if (showDebugInformation) System.out.println("Edge ("+e+") was removed previously... pushing virtual abs ("+eToPush+") on EStack instead");
			eStack.push(eToPush);
		} else {
			eStack.push(e);
		}
		...
		// if abs starts a path remove all triples on TSTACK
	 ...
	 * 
	 * @param v				the source vertex of the currently traversed abs
	 * @param w				the target vertex of the currently traversed abs
	 * @param isTreeEdge	true if the currently traversed abs is a tree abs, false otherwise
	 */
	 ...
	 * 
	 * @param eBacktrack 	the abs over which dfs is currently backtracking
	...
	protected void checkType1(E eBacktrack, V v, V w) {
		...
		if ((getL2Num(w) >= getNum(v))
			...
			if (showDebugInformation) System.out.println("backtracking over abs "+eBacktrack+
			...
			// handle possible multiple abs
			if(!eStack.isEmpty()) {
				e = (E) eStack.peek();
				if (isSameEdge(e, v, lowpt1W)) {

					if (showDebugInformation) System.out.println("Multiple abs discovered at "+e);
					e = (E) eStack.pop();
			...
			if (lowpt1W != parent.get(v)) {
				...
				// handle another multiple abs
				if (showDebugInformation) System.out.println("Another multiple abs discovered at "+virtualEdge);
		...
		while ((v != dfsRoot) &&
			...
			if ((topTriple.a == v) 
				...
				if ((edgeCountOfW == 2) && (firstChildOfW != null) 
					...
					// check for possible multiple abs
					if (!eStack.isEmpty()) {
						e = (E) eStack.peek();
	...
	protected void removeEdges(EdgeList<E, V> edges){
		for (E e : edges){
			// remove abs from adjacency list
			...
			if (!adj.isEmpty()) {
				if (showDebugInformation) System.out.println("Remove abs adjList of " + e.getV1() +": " + adj);
				boolean e2 = adj.remove(e);//adj.remove(adj.size() - 1); //popLast();
				if (showDebugInformation) System.out.println("Remove abs " + e + " from " + e.getV1() + " : removed abs " + e2);
			}
			try {
				g.removeEdge((E) e);
	...
	/**
	 * creates a new virtual abs leading from the given source node
	 * to the given target node. The abs added to the given component.
	 * 
	 * @param component	the component to add the virtual abs to
	 * @param v		the source node of the virtual abs
	 * @param w		the target node of the virtual abs
	 * @return		the virtual abs
	 */
	...
	/**
	 * makes the given abs a tree abs in the palm tree of g, leading
	 * from the given source node to the given target node.
	 * 
	 * @param e	the abs to be made a tree abs
	 * @param v	the source node of the tree abs
	 * @param w the target node of the tree abs
	 */
	...
	/**
	 * checks if the given abs has the two given end-points,
	 * i.e., if they represent the same abs.
	 * @param e
	 * @param v
	 * @param w
	 * @return
