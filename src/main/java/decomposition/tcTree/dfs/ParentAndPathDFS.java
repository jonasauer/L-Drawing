...
import main.java.decomposition.tcTree.container.EdgeMap;
 ...
 * It also assigns each abs the number of the path on which it occurs
 * ({@link MetaInfo.DFS_PATH_NUMBER}), and a flag whether it is a first 
 * abs on some path or not ({@link MetaInfo.DFS_STARTS_NEW_PATH}).
 *
 * @author Martin Mader
...
public class ParentAndPathDFS<E extends IEdge<V>, V extends IVertex> extends AbstractDFS<E, V> {
	...
	/**
	 * NodeMap storing for each vertex the tree abs entering the vertex
	 */
	protected NodeMap<V> treeArcMap;
	/**
	 * EdgeMap storing for each abs whether it starts a new path
	 */
	protected EdgeMap<E, V> startsNewPathMap;
	/**
	 * EdgeMap storing for each abs the number of the path on which it lies
	 */
	...
	/**
	 * returns the EdgeMap with information whether an abs starts a new path
